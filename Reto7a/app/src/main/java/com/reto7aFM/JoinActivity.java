                    ...
                    if(dataSnapshot.child(code[0]).exists()) {
                        if(dataSnapshot.child(code[0]).child("p2").exists()) {
                            Toast.makeText(getBaseContext(),
                                    "el juego ya comenzó por favor genera un nuevo codigo.",
                            ...
                            if(dataSnapshot.child(code[0]).child("start_time")
                                ...
                                Toast.makeText(getBaseContext(),
                                        "El codigo ha expirado por favor crea un nuevo codigo.",
                                        Toast.LENGTH_LONG).show();
                            } else {
                                ...
                                if(android_id.equals(dataSnapshot.child(code[0]).child("p1")
                                    ...
                                    Toast.makeText(getBaseContext(),
                                            "no puedes iniciar un juego iniciado por ti.",
                                            Toast.LENGTH_LONG).show();
                                } else {
                                    myRef.child(String.valueOf(code[0])).child("p2")
                                            .setValue(android_id);
                                    Toast.makeText(getBaseContext(), "Comencemos a jugar!",
                                            Toast.LENGTH_LONG).show();
                        ...
                        Toast.makeText(getBaseContext(), "Codigo invalido!", Toast.LENGTH_SHORT).show();
                    }
                }
            }
        });
